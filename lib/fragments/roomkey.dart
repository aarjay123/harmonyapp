...
import '../helpcenter/articles/roomkey_help_page.dart';

class RoomkeyFragment extends StatefulWidget {
  const RoomkeyFragment({super.key});

  @override
  State<RoomkeyFragment> createState() => _RoomkeyFragmentState();
}

class _RoomkeyFragmentState extends State<RoomkeyFragment> {
  ...
  bool _isUnlocked = false; // kept this from intermediate version just in case, but logic follows original
  ...
  void _toggleLock() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(_isUnlocked ? 'Room Unlocked successfully.' : 'Room Locked'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
          SliverAppBar.large(
            title: Text('Room Key', style: TextStyle(color: colorScheme.onSurface)),
            backgroundColor: colorScheme.surface,
            pinned: true,
            actions: [
              IconButton(
                icon: const Icon(Icons.help_outline_rounded),
                onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const RoomKeyHelpPage())),
              ),
            ],
              ...
              background: Container(
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: [Colors.amber.withOpacity(0.3), colorScheme.surface],
                    begin: Alignment.topCenter,
                    end: Alignment.bottomCenter,
                  ),
                ),
                child: Center(
                  child: Icon(Icons.vpn_key_rounded, size: 80, color: Colors.amber.withOpacity(0.5)),
                ),
              ),
            ),
          ),
              ...
              child: Center(
                child: ConstrainedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      // Header Text (Optional secondary header if you want it below the SliverAppBar)
                      Text(
